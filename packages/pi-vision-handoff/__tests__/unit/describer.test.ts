...
vi.mock("@earendil-works/pi-ai/compat", () => ({
  complete: (...args: unknown[]) =>
    complete(...(args as Parameters<typeof complete>)),
}));
...
const img = (data: string, mimeType = "image/png"): ExtractedImage => ({
  data,
  mimeType,
});

function fakeResponse(
  opts: Partial<AssistantMessage> & { text?: string },
): AssistantMessage {
...
describe("describeSingle stopReason handling", () => {
  ...
  it("returns the raw text on a clean stop", async () => {
    complete.mockResolvedValueOnce(
      fakeResponse({ text: "full description", stopReason: "stop" }),
    );
    const out = await describeSingle(
      img("AAA"),
      "",
      visionModel,
      modelRegistry,
  ...
  it("appends the truncation marker on stopReason length (not silent)", async () => {
    complete.mockResolvedValueOnce(
      fakeResponse({
        text: "partial description cut off mid-sente",
        stopReason: "length",
      }),
    );
    const out = await describeSingle(
    ...
    expect(out).toBe(
      `partial description cut off mid-sente${DESCRIPTION_TRUNCATED_MARKER}`,
    );
  });
  ...
  it("returns null (no marker) on an aborted/error stop, surfacing the error", async () => {
    const deps = makeDeps();
    complete.mockResolvedValueOnce(
      fakeResponse({ stopReason: "error", errorMessage: "boom" }),
    );
    const out = await describeSingle(
      img("AAA"),
      "",
  ...
  it("returns null for an empty text response and surfaces an empty-description error", async () => {
    const deps = makeDeps();
    complete.mockResolvedValueOnce(
      fakeResponse({ text: "", stopReason: "stop" }),
    );
    const out = await describeSingle(
      img("AAA"),
      "",
  ...
  it("passes the vision model's declared maxTokens when cfg.maxTokens is unset", async () => {
    complete.mockResolvedValueOnce(
      fakeResponse({ text: "ok", stopReason: "stop" }),
    );
...
describe("resolveMaxTokens", () => {
  ...
  const cfg = (maxTokens?: number): VisionHandoffConfig => ({
    ...baseCfg,
    maxTokens,
  });
  ...
  it("a configured cfg.maxTokens wins and is also clamped to the window", () => {
    ...
    expect(resolveMaxTokens(cfg(200000), model(262144, 131072))).toBe(
      131072 - 8192,
    );
  });
...
describe("runBatch stopReason handling", () => {
  ...
  it("marks only the LAST parsed image on stopReason length (the one cut off mid-stream)", async () => {
    ...
    expect(out.get(hc)).toBe(
      `desc three partial cut${DESCRIPTION_TRUNCATED_MARKER}`,
    );
  });
