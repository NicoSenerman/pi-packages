      ...
      sock.on("close", () => {
        ...
        if (!this.closed) {
          debugLog(
            ...
            "connection closed; will reconnect on next send",
          );
        }
      });
    });
...
export class AgentEventObserver {
  ...
  onSubagentStarted(record: Subagent): void {
    ...
    this.retrySubscribe(record);
    if (!this.subscriptions.has(record.id)) this.scheduleRetry(record);
  }
   ...
   *  mid-run, so subscribeToUpdates returns undefined at created/started time.
   *  retrySubscribe() is called from lifecycle hooks and retries until the
   *  session exists; scheduleRetry() covers the gap when no further hook
   *  fires before the session is ready. */
  private attach(agent: Subagent): void {
    this.cancelObserver(agent.id);
    this.retrySubscribe(agent);
    if (!this.subscriptions.has(agent.id)) this.scheduleRetry(agent);
  }

  private retrySubscribe(agent: Subagent): void {
  ...
  private scheduleRetry(agent: Subagent): void {
    setTimeout(() => {
      if (this.subscriptions.has(agent.id)) return;
