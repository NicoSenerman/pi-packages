...
describe("SubagentState — thinking", () => {
  it("defaults to an empty string", () => {
    const state = new SubagentState();
    expect(state.thinking).toBe("");
  });

  it("appendThinking concatenates thinking deltas", () => {
    const state = new SubagentState();
    state.appendThinking("Let me ");
    state.appendThinking("think");
    expect(state.thinking).toBe("Let me think");
  });

  it("resetThinking clears accumulated thinking", () => {
    const state = new SubagentState();
    state.appendThinking("previous thinking");
    state.resetThinking();
    expect(state.thinking).toBe("");
  });
});
