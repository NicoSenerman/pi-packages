...
// Passthrough theme stub used across widget tests: fg/bold return text unchanged.
const stubTheme = {
...
function makeAgent(
  overrides: { id?: string; status?: string; completedAt?: number } = {},
) {
  return {
    id: "agent-1",
    status: "completed",
...
describe("AgentWidget — projection reads activity off Subagent records", () => {
  it("surfaces turnCount, activeTools, and responseText from the record via renderWidget", () => {
    ...
    let renderFn:
      ((tui: unknown, theme: unknown) => { render(): string[] }) | undefined;
    const ui: UICtx = {
      setStatus: () => {},
      theme: stubTheme,
      setWidget: (_key, content) => {
        if (typeof content === "function")
          renderFn = content as typeof renderFn;
      },
    };
    widget.setUICtx(ui, "tui");
    widget.update();
...
describe("AgentWidget.update self-seeds finished agents", () => {
  ...
  function makeWidget(
    ...
    const manager = {
      listAgents: () =>
        agents.map((a) => ({ invocation: { runInBackground: true }, ...a })),
    } as unknown as SubagentManager;
    const registry = new AgentTypeRegistry(() => new Map());
    const widget = new AgentWidget(manager, registry);
  ...
  it("lingers an error agent for two turns before aging out", () => {
    const { widget, lastContent } = makeWidget([
      { id: "a1", status: "error", completedAt: 5000 },
    ]);
    widget.update();
    expect(typeof lastContent()).toBe("function");
...
describe("AgentWidget — self-drives from lifecycle notifications", () => {
  ...
  it("starts the update timer and renders on onSubagentStarted", () => {
    ...
    widget.onSubagentStarted(
      createTestSubagent({ id: "a1", status: "running" }),
    );
  ...
  it("starts the update timer and renders on onSubagentCreated", () => {
    ...
    widget.onSubagentCreated(
      createTestSubagent({ id: "a1", status: "queued" }),
    );
  ...
  it("renders the finished agent on onSubagentCompleted", () => {
    ...
    widget.onSubagentCompleted(
      createTestSubagent({ id: "a1", status: "completed" }),
    );
...
describe("AgentWidget — non-TUI (rpc) mode emits a pre-rendered string[]", () => {
  // In non-TUI modes pi drops factory-form setWidget; the widget must instead
  // push a pre-rendered string[] each tick so the RPC bridge forwards it to the
  // external TUI host. The 80ms spinner interval re-emits via update().
  function makeRpcWidget(records: Subagent[]) {
    const manager = { listAgents: () => records } as unknown as SubagentManager;
    const registry = new AgentTypeRegistry(() => new Map());
    const widget = new AgentWidget(manager, registry);
    const setWidgetCalls: Array<{ content: unknown; placement?: string }> = [];
    const ui: UICtx = {
      setStatus: () => {},
      setWidget: (_key, content, options) => {
        setWidgetCalls.push({ content, placement: options?.placement });
      },
      theme: stubTheme,
    };
    widget.setUICtx(ui, "rpc");
    const lastContent = () => setWidgetCalls.at(-1)?.content;
    return { widget, lastContent, setWidgetCalls };
  }

  it("emits a string[] (not a factory) with aboveEditor placement", () => {
    const { widget, lastContent, setWidgetCalls } = makeRpcWidget([
      createTestSubagent({
        id: "a1",
        status: "running",
        completedAt: undefined,
    ...
    expect(typeof lastContent()).not.toBe("function");
    expect(setWidgetCalls.at(-1)?.placement).toBe("aboveEditor");
  });

  it("re-emits on each update() so the spinner frame advances", () => {
    const { widget, setWidgetCalls } = makeRpcWidget([
      createTestSubagent({
        id: "a1",
        status: "running",
        completedAt: undefined,
    ...
    widget.update();
    widget.update();
    const stringArrayCalls = setWidgetCalls.filter((c) =>
      Array.isArray(c.content),
    );
    expect(stringArrayCalls.length).toBeGreaterThanOrEqual(2);
  });

  it("renders lines through the theme (stub returns text unchanged)", () => {
    const { widget, lastContent } = makeRpcWidget([
      createTestSubagent({
        id: "a1",
        status: "running",
        completedAt: undefined,
    ...
    widget.update();
    const lines = lastContent() as string[];
    expect(lines.some((l) => l.includes("Agents"))).toBe(true);
  });

  it("clears the widget with undefined when idle", () => {
    const { widget, lastContent } = makeRpcWidget([
      createTestSubagent({
        id: "a1",
        status: "completed",
    ...
    widget.update();
    expect(Array.isArray(lastContent())).toBe(true);
    // Age the finished agent out; completed agents linger exactly 1 turn.
    widget.onTurnStart();
    expect(lastContent()).toBeUndefined();
  });
});
