...
Everything else (`index.ts`, `neuralwatt-mcr.ts`, `transform.ts`, `chad-mcr-upstream.ts`, `custom-models.json`, `patch.json`, `docs/`, `scripts/`, `tests/`, `README.md`, `AGENTS.md`, `package.json`, `knip.json`, `vitest.config.ts`, `.gitignore`, `.npmignore`, `LICENSE`) is **byte-for-byte identical to upstream v1.7.3**.
...
The extension registers `api: "neuralwatt"` with a custom `streamSimple: streamNeuralwatt` handler. This is **the mechanism that captures energy / cost / MCR data**: it overrides `globalThis.fetch`, tees the SSE response body (`response.body.tee()`), feeds one half to `readEnergyFromTee()` while the OpenAI SDK consumes the other, and parses the `: energy` / `: cost` / `: mcr-session` SSE comment lines the SDK would otherwise discard.
...
| `Cannot find module '@mariozechner/pi-coding-agent'` (chad-mcr-upstream.ts:1)                                       | `chad-mcr-upstream.ts` imports the _old_ package name `@mariozechner/pi-coding-agent` (now `@earendil-works/pi-coding-agent`). Upstream's own `sync-mcr` script fetches this file verbatim from `neuralwatt-tools`; it's a vendored upstream companion.                                                                                                                                                                                                                                                                                                                                              | **No** — would diverge from upstream's synced file.                                                                                                                 |
| `Cannot find module 'typebox'` (chad-mcr-upstream.ts:6)                                                             | declared in upstream `package.json` devDeps but not installed (no `pnpm install` in the fork).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **No** — dev-only; affects `chad-mcr-upstream.ts` only.                                                                                                             |
...
If `index.ts` / `neuralwatt-mcr.ts` change the `NeuralwattConfig` schema or the `buildModels` pipeline, re-read the new versions and update this `FORK.md`'s Step C / Section 1 accordingly.
