...
export const DEFAULT_PROMPT_GUIDELINES: string[] = [
	...
	"`next_task` — recommend the next open, unblocked task (highest priority, lowest id); use it to pick what to work on next.",
];
...
export function registerTodoTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		label: TOOL_LABEL,
		description:
			"Manage a task list for tracking multi-step progress. Actions: create (new task), update (change status/fields/dependencies), list (all tasks, optionally filtered by status), get (single task details), delete (tombstone), clear (reset all), next_task (recommend the next open unblocked task). Status: pending → in_progress → completed, plus deleted tombstone. Use this to plan and track multi-step work like research, design, and implementation.",
