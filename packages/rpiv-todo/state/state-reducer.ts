...
export function applyTaskMutation(state: TaskState, action: TaskAction, params: TaskMutationParams): ApplyResult {
	switch (action) {
		...
		case "next_task": {
			// Pure query — does not mutate state. Among pending tasks whose every
			// blockedBy dependency is completed, pick the highest priority (then
			// lowest id). Reuses the `get` op so the response envelope renders the
			// chosen task via `formatGetLines` (no new Op variant needed).
			...
			const candidates = state.tasks.filter(isReady);
			if (candidates.length === 0) {
				return errorResult(state, "no open tasks ready to start");
			}
			let chosen = candidates[0];
			for (const task of candidates) {
				const a = task.priority ?? 0;
				const b = chosen.priority ?? 0;
				if (a > b || (a === b && task.id < chosen.id)) chosen = task;
			}
			return { state, op: { kind: "get", task: chosen } };
		}
	}
}
