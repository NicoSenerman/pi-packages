...
export interface RenderSingleSelectRowsParams {
  ...
  /** Inline freeform text typed into the "Type something." row (issue #3). */
  freeformText?: string;
  /** Whether the inline freeform field is actively capturing input. */
  freeformActive?: boolean;
  /**
   * Optional description renderer for full styled markdown in the left
   * column. When provided, replaces the plain `wrapText` path so descriptions
   * render as real markdown (bold, code, headings) instead of literal
   * `**bold**` text. Receives the description text + available wrap width;
   * returns the rendered lines (ANSI-styled) already wrapped.
   */
  renderDescription?: (text: string, width: number) => string[];
}
...
function buildItemBlocks(
  ...
  const freeformLabel = freeformActive ? `${freeformText}▏` : "Type something.";
  ...
  return allItems.map((item, itemIndex) => {
    ...
    if (item.option.description && !hideDescriptions) {
      ...
      const descWidth = Math.max(8, normalizedWidth - descriptionPrefix.length);
      const descriptionLines = renderDescription
        ? renderDescription(item.option.description, descWidth)
        : wrapText(item.option.description, descWidth);
